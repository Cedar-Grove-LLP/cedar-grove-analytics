...
import { buildProjectedEarningsRows, sumProjectedTotals } from '@/utils/projectedEarnings.mjs';
...
const EarningsCard = ({ title, rows, togglePromote }) => {
  ...
  const totals = sumProjectedTotals(rows);
...
const ProjectedEarningsTable = () => {
  ...
  // All projection math lives in the pure engine (utils/projectedEarnings.mjs);
  // this component only wires Firestore data + the Promote toggles into it.
  const rows = useMemo(
    () =>
