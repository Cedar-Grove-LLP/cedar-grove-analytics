...
└── utils/
    ...
    ├── rateLookup.mjs      # Pure billing-rate lookup (backward fallback + retrospective earliest-rate fallback for pre-history months, tested)
...
- **Manual bill adjustments (McClure only):** Sam McClure's timesheet has an **Adjustment ($)** column (current-layout col D) for month-end charges (+) / credits (−) to a client's final bill **without logging time**. It is folded into **Billables Earnings** (col E = `Hours × Rate + Adjustment`), so synced `earnings` already includes it while hours columns and `Total Billable Hours` stay untouched. The dashboard normalizes a per-entry `adjustment` field (defaults to 0; absent on other attorneys and legacy tabs) and surfaces an **Adjustments** KPI + a Recent Entries column on the attorney detail page — it is informational only, **never added on top of `earnings`** (already inside). The **Billing Summaries** invoice-prep page is the one place it IS added on: its Amount column is `rate × hours + adjustment` (calc key `billingSummaryAmount`, mirroring the sheet's Billables Earnings construction), pure adjustment rows (0 hours) are included on the bill, and an Adjustment column/total appears whenever the selection has one. Calc keys: `adjustment`, `entryAdjustment`, `billingSummaryAmount`. Apps Script steps: `docs/timesheet-sync.md`.
...
- Tailwind utility classes for all styling. Custom brand colors defined in `globals.css` `@theme` (`cg-black`, `cg-green`, `cg-dark`, `cg-background`, plus `cg-green-text` — a darkened green for TEXT/links, since raw `cg-green` fails WCAG 4.5:1 text contrast; keep `cg-green` for fills/icons/borders). The gray ramp's `gray-500` is deliberately darker than visually implied so `text-gray-500` passes 4.5:1; use `text-green-700` (not `green-600`) for green text like currency amounts.
